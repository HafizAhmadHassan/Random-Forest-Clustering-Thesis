...
def unsuperv_rf_proximity(name,j,dir):
    ...
    arr = ["BreastTissues.csv", "auto-mpg.csv", "Lung.csv", "Parinkson.csv", "iris.csv", "glass-e.csv", "heart.csv", "WBC.csv", "wine.csv"]
    ...
    for val in arr:
        ...
        RF.append(rf)
        sRF.append(srf)
        pred_actual.append(list(c_df[i].y))
...
if __name__ == '__main__':
    ...
    for k in dr:
        for j in range(0, 1):
            ...
            df_raw.append(readAndSavefile("datasets/", "heart.csv"))
            #pred_actual.append(list(df_raw[0].y))

            c_df.append(semi_supds(df_raw[0], 0.3))
            print(c_df)
            ...
            RF.append(rf)
            sRF.append(srf)
            pred_actual.append(list(c_df[0].y))

            Prm.append(proximityMatrix(RF[i], c_df[i].drop("y", axis=1), normalize=True))
            dm.append(1 - Prm[i])
            dm[0] = dm[i][:int(np.shape(dm[0])[0]), :int(np.shape(dm[0])[0])]
            dm[0] = preprocessing.normalize(dm[0])
            print(dm[0])

            cluster_c.append(AgglomerativeClustering(n_clusters=3,
                                                         affinity='precomputed',
                                                         linkage='complete'))
            cluster_c[i].fit_predict(dm[0])
            cluster_c[i].labels_ = cluster_c[i].labels_ + 1
            sfC.append(sklearn.metrics.adjusted_rand_score(pred_actual[i], cluster_c[i].labels_))
            ...
            try:
                s_cluster_s.append(AgglomerativeClustering(n_clusters=3,
                                                           affinity='precomputed',
                                                           linkage='single'))
                s_cluster_s[i].fit_predict(dm[0])
                s_sfS.append(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_s[i].labels_))

                print(str(round(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_s[i].labels_), 4)))
                print(",")
            except:
                print("0.0")
                print(",")
            ...
            try:
                s_cluster_spec.append(
                    SpectralClustering(n_clusters=3, affinity="rbf", assign_labels="discretize"))
                s_cluster_spec[i].fit_predict(dm[0])
                pred_actual[i] = np.array(pred_actual[i])
                s_sfSpec.append(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_spec[i].labels_))

                print(str(round(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_spec[i].labels_), 4)))
                print(",")
            except:
                print("0.0")
                print(",")
            ...
            try:
                s_cluster_alg.append(AgglomerativeClustering(n_clusters=3))
                s_cluster_alg[i].fit_predict(dm[0])
                pred_actual[i] = np.array(pred_actual[i])

                s_sfAlg.append(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_alg[i].labels_))

                print(str(round(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_alg[i].labels_), 4)))
                print(",")
            except:
                print("0.0")
                print(",")
            ...
            try:
                ...
                s_cluster_ap[i].fit_predict(dm[0])
                pred_actual[i] = np.array(pred_actual[i])
                s_sfAp.append(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_ap[i].labels_))
                print(str(round(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_ap[i].labels_), 4)))
                print(",")
            except:
                print("Exception")
                print("0.0")
            ...
            try:
                ...
                s_cluster_p[i].fit_predict(dm[0])
                pred_actual[i] = np.array(pred_actual[i])
                s_sfKmed.append((sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_p[i].labels_)))
                print(str(round(sklearn.metrics.adjusted_rand_score(pred_actual[i], s_cluster_p[i].labels_), 4)))
                print("\n")
            except:
                print("0.0")
                print("\n")
