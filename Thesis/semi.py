...
def unsuperv_rf_proximity(name, j, dir):
    ...
    for val in arr:
        ...
        c_df.append(semi_supds(df_raw[i], float(dir)))
        print("Hassan")

        if "Unnamed: 0" in df_raw[i].columns:
            temp = []
            temp = df_raw[i].drop("Unnamed: 0", axis=1)
            print(temp, "sssss")
            df_raw[i] = []
            df_raw[i] = temp

        if "Unnamed: 0" in c_df[i].columns:
            temp = []
            temp = c_df[i].drop("Unnamed: 0", axis=1)
            c_df[i] = []
            c_df[i] = temp
        ...
        if (len(c_df[i].columns) != len(df_raw[i].columns)):
            print("See")
            print(df_raw[i])
            print(c_df[i])
