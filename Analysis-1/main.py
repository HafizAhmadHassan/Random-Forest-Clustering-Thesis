...
def unsuperv_rf_proximity(name,j,dir):
    ...
    for val in arr:
        ...
        tl.append(Tree_leaf_weight(y[i], SmDt[i]))
        ...
        pmt[i] = 1-pmt2[i]
        np.square(pmt[i])
        #pmt[i] = preprocessing.normalize(pmt[i])
        ...
        try:
            s_cluster_c.append(AgglomerativeClustering(n_clusters=clus[i],
                                                     affinity='precomputed',
                                                     linkage='complete'))
            s_cluster_c[i].fit_predict(pmt[i])
        ...
        try:
            s_cluster_s.append(AgglomerativeClustering(n_clusters=clus[i],
                                                     affinity='precomputed',
                                                     linkage='single'))
            s_cluster_s[i].fit_predict(pmt[i])
        ...
        try:
            ...
            s_cluster_spec[i].fit_predict(pmt[i])
            pred_actual[i] = np.array(pred_actual[i])
        ...
        try:
            ...
            s_cluster_alg[i].fit_predict(pmt[i])
            pred_actual[i] = np.array(pred_actual[i])
        ...
        try:
            ...
            s_cluster_ap[i].fit_predict(pmt[i])
            pred_actual[i] = np.array(pred_actual[i])
        ...
        try:
            ...
            s_cluster_p[i].fit_predict(pmt[i])
            pred_actual[i] = np.array(pred_actual[i])
...
if __name__ == '__main__':
    df1 = pd.read_excel("output 0.1.xlsx", sheet_name="Sheet1")
    df2 = pd.read_excel("output 0.3.xlsx", sheet_name="Sheet1")
    df3 = pd.read_excel("output 0.5.xlsx", sheet_name="Sheet1")
    df4 = pd.read_excel("output 0.7.xlsx", sheet_name="Sheet1")
    df5 = pd.read_excel("output 0.9.xlsx", sheet_name="Sheet1")

    print(df1.loc[0],df2.loc[0],df3.loc[0],df4.loc[0],df5.loc[0])